...
- **Status (2026-03-21):** Cloud PostgreSQL (Supabase) provisioned. Master credentials stored in local `.env`.

### 4. Master Data Management: Medical Ontology (Phase 5 SaaS Roadmap)
- **Implementation Plan:** Decoupling hardcoded diseases into a globally unified PostgreSQL `disease_criteria` layer. We will query NIH UMLS APIs to systematically cross-walk SNOMED, ICD-10, and MeSH formats. This empowers the Next.js UI with universal auto-complete inputs and unlocks lucrative B2B Data-as-a-Service (DaaS) API productization for external EHR/CRO vendors.

### 5. Architectural Dictionary Binding
- **Implementation:** The backend relies heavily on distinct execution node logic. To safely abstract this complexity without hardcoding arbitrary acronyms, a permanent Excel registry (`meddash_scripts_dictionary.xlsx`) was engineered mapping scripts explicitly to their physical components.
- **Rule:** *Whenever a change is made to the SaaS underlying components or a new python script is introduced, the dictionary matrix (`meddash_scripts_dictionary.xlsx`) MUST be updated with a formal date/time stamp and explicit reason for the modification.*
