...
*   **Database Interaction:** `psycopg2` or `asyncpg` driver in Python (FastAPI) for PostgreSQL.
...
*   **Purpose:** A web-based equivalent to "pgAdmin 4" for PostgreSQL.
*   **Features:**
...
### 5. Campaign Sandbox (Pull ID) Validation Bay
*   **Purpose:** Isolate newly scraped clinical/KOL data into a staging environment to prevent duplicate contamination in the global database.
*   **Features:**
    *   Target scrapes inject a `pull_id` argument to group results into a PostgreSQL staging table.
    *   Data Table visualization of pending/staged KOLs for a designated Client Campaign.
    *   Macro Execution Buttons: `[Run Disambiguation]`, `[Export Client Text Report]`, `[Commit to Global DB]`, and `[Erase Sandbox]`.
...
2.  **Phase 2: Database Visualizer & PostgreSQL Migration**
    *   Migrate centralized datastores from SQLite to a unified PostgreSQL cluster to support multi-node multi-agent simultaneous access.
    *   Build the FastAPI endpoints to read from the 3 PostgreSQL databases.
...
> [!NOTE]
> Review this architectural plan. Because your entire backend is Python and PostgreSQL based, wrapping it with a small FastAPI server is the safest, most robust way to allow a beautiful Next.js frontend to securely command your scripts.

---

### 6. Multi-Tier KOL Disambiguation Workflow (Human-in-the-Loop)
*   **Purpose:** Systematically resolve duplicate KOL profiles across Campaign Sandbox pulls using a 3-tier escalation model before committing to the global database, preventing bloat from redundant clones.
*   **Tier 1 — Automatic Verification (Engine):**
    *   `kol_disambiguator.py` scores all KOL pairs using 4 weighted signals (Co-author 40%, MeSH 30%, Name 25%, Temporal 5%).
    *   ORCID-matched pairs (score 1.0) → auto-merged and marked `verified`.
    *   High-confidence pairs (score ≥ 0.70) → auto-merged and marked `verified`.
    *   Standalone unique KOLs (no duplicates found) → auto-marked `verified`.
    *   Uncertain pairs (flagged but < threshold) → left as `pending` in `kol_merge_candidates`.
*   **Tier 2 — Gemini Flash HITL Review (Human + AI):**
    *   A dedicated HITL panel on the Sandbox page renders remaining `pending` pairs.
    *   Side-by-side KOL comparison cards display names, institutions, specialties, MeSH tags, and co-authors.
    *   "Ask Gemini" button queries Gemini Flash 2.5 for contextual reasoning (e.g., "Common Chinese surname, different specialties, likely two individuals").
    *   Human resolves: `[Same Person]` → merge, `[Two People]` → confirm distinct, `[Escalate]` → move to Tier 3.
*   **Tier 3 — Deep Disambiguation Queue (Manual Research):**
    *   Escalated pairs land in `deep_disambiguation_needed` PostgreSQL table.
    *   These require reading full publication texts, comparing research methodologies, and manually sorting.
    *   Expected volume: very low (< 1% of total KOLs) after Tier 1 and Tier 2 filtering.
*   **System Health Integration:**
    *   A new "Sandbox Disambiguation" tab in System Health Override streams `sandbox_dedup_{pull_id}.log` in real-time.
*   **Database Tables:**
    *   `kol_merge_candidates` — Tier 1/2 scoring queue (existing).
    *   `deep_disambiguation_needed` — Tier 3 escalation table (new).
*   **API Routes:**
    *   `GET /api/sandbox/pending_pairs?pull_id=XYZ` — Fetch pending merge candidates.
    *   `POST /api/sandbox/gemini_review` — Query Gemini Flash 2.5 for pair analysis.
    *   `POST /api/sandbox/resolve_pair` — Resolve a pair (same_person / two_people / escalate).
